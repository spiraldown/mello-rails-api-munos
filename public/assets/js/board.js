...
const $boardConatiner = $('.container');
...
function createCards(list) {
  ...
  let $cardList = list.cards.map(function(card) {
    let $cardList  = $('<li>');
...
function renderBoard() {
  ...
  $boardConatiner.empty();
  $boardConatiner.append($lists);
...
function renderContributors() {
  let $contributorListItems = board.users.map(function(user) {
    ...
    let $contributerDeleteButton = $('<button class= "danger">Remove</button>')
      .data(user)
      .on('click', handleContributorDelete);

    $contributorListItem.append($contributorSpan, $contributerDeleteButton);
...
function makeSortable(){
  Sortable.create($boardConatiner[0], {
    animation: 300,
    ghostClass: 'ghost',
    filter: '.add',
